...
isPlayerStealthed:	.word	0	#1 => player is invisible, 2 => player is NOT invisible
...
U5M3_OUTER_START:
	...
	li	$a1, 1	#x-counter		#reset x-counter each iteration
U5M3_INNER_START:
	beq	$a1, 14, U5M3_INNER_END		#branch when x == 14
...
U5M3_OUTER_END:
	...
	#add some extra blocks beside U5M3
	li	$a0, 3620
	li	$a1, BORDER_COLOUR
	addi	$a0, $a0, BASE_ADDRESS
	sw	$a1, ($a0)
...
enemyCollision:
	...
	beq 	$a1, LAVA, enemyCollisionTrue
...
#F: trigger gameOver if health reaches 0 
displayGameOver:
...
nextLevel:
	...
	lw	$a0, ($a0)		#load first playerModel offset
	addi	$a0, $a0, BASE_ADDRESS	#compute bitmap address
	addi	$a0, $a0, 4		
...
advanceToWin:		#to be completed

nextLevelEnd:
	jr	$ra		#return to gameLoop

#checks if playerModel has picked up a powerup
checkPickup:
	#push ra to stack
	addi	$sp, $sp,-4
	sw	$ra, ($sp)	
